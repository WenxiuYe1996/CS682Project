...
def getArticleCount(url):
    # Find the number of articles found from the given url
...
def getPDFNameFromGivenURL(url):
    # Get all the number file name from the given url
    page = requests.get(url)
    doc = BeautifulSoup(page.text, "html.parser")
    c = doc.find('script', id="serverApp-state").text
    result_list = re.findall('&q;/(.+?)&q;', c)
...
def getDPFLink(result_list):
    # Get all the links to articles and links to the description of the ararticles
    pdf_link_list = []
    link_list = []
...
def search(userInputTopic, lang="en"):
    ...
    paper_links_path = f'{csvDirName}/paper_links.csv'
    paper_links.to_csv(paper_links_path, quoting=csv.QUOTE_NONE, quotechar='',escapechar='\\')
    # Generate a table for paper_links in HTML
    ...
    return_list  = []
    return_list.append(f'Number of paper Downloaded: {total_num_paper}')

    table_name_list = []
    table_name_list.append(paper_links_html)
